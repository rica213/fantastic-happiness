...
RSpec.describe 'Post', type: :feature do
  ...
  describe 'Testing Post/show', type: :feature do
    before(:each) do
      ...
      @user2 = User.create(name: 'Larissa', photo: 'www.unsplash.com', bio: 'Bee keeper and bee lover')

      @first_post = Post.create(author: @user1, title: 'Hello World', text: 'This is my first post')

      @first_comment = Comment.create(author: @user1, post: @first_post, text: 'First comment for the post')
      @second_comment = Comment.create(author: @user1, post: @first_post, text: 'Second comment for the post')
      @third_comment = Comment.create(author: @user1, post: @first_post, text: 'Third comment for the post')
    ...
    it 'displays the number of the comments for the post' do
      expect(page).to have_content(@first_post.comments_count)
    end

    it 'displays the number of the likes for the post' do
      expect(page).to have_content(@first_post.likes_count)
    end
